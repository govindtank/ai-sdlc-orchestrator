...
def generate_test_cases_from_user_story(user_story_id: int, db: Session = Depends(get_db)):
    ...
    test_cases_result = qa_service.generate_test_cases(story_dict)
...
@router.post("/{user_story_id}/suggest-test-data", response_model=schemas.TestDataResponse)
def suggest_test_data_for_user_story(user_story_id: int, db: Session = Depends(get_db)):
    """
    ...
    # Prepare user story dict for test data suggestion
    story_dict = {
        "title": user_story.title,
        "description": user_story.description,
        "acceptance_criteria": user_story.acceptance_criteria.split("\n") if user_story.acceptance_criteria else []
    ...
    # Suggest test data using the service
    test_data_result = qa_service.suggest_test_data(story_dict)
